...
const SECTIONS = JSON.parse(readFileSync(join(ROOT, 'src', 'data', 'landings-sections.json'), 'utf-8'));

/**
 * Sections longues + tableau d'une landing, en HTML crawlable.
 * DOIT rester le miroir de ce que rend ToolLanding.tsx : le bot-content est
 * masque et retire au montage, donc toute divergence serait du cloaking.
 */
const landingSections = (slug, lang) => {
  const c = SECTIONS[slug] && SECTIONS[slug][lang];
  if (!c) return '';
  // Resume extractable en tete : reponse directe dans les 40-60 premiers mots.
  const summary = c.summary ? `<p>${escHtml(c.summary)}</p>` : '';
  const secs = c.sections
    .map((s) => `<h2>${escHtml(s.h2)}</h2>${s.p.map((t) => `<p>${escHtml(t)}</p>`).join('')}`)
    .join('\n');
  const t = c.table;
  const table = t
    ? `<table><caption>${escHtml(t.caption)}</caption><thead><tr>${t.headers
...
const T = {
  ...
  toolsIntro: {
    fr: "18 outils gratuits en navigateur pour retirer les Content Credentials C2PA, l'EXIF/GPS et les empreintes IA des images générées par Sora, Midjourney, DALL-E, ChatGPT, Firefly, Flux et plus. Sans upload.",
    en: '18 free in-browser tools to remove C2PA Content Credentials, EXIF/GPS and AI fingerprints from images generated by Sora, Midjourney, DALL-E, ChatGPT, Firefly, Flux and more. No upload.',
  },
};
...
for (const lang of ['fr', 'en']) {
  ...
  // ── / ──
  {
    ...
    const body = [
      ...
      // Les 3 etapes et les 2 colonnes d'architecture sont rendues par Home.tsx
      // mais etaient absentes du bot-content : les crawlers sans JS voyaient des
      // titres sans contenu (157 mots sur la page la plus strategique du site).
      ...[1, 2, 3].map(
        (i) =>
          `<h3>${escHtml(get(L, `info.step_${i}_title`))}</h3><p>${escHtml(get(L, `info.step_${i}_desc`))}</p>`
      ),
      `<h2>${escHtml(get(L, 'info.arch_title'))} ${escHtml(get(L, 'info.arch_highlight'))}</h2>`,
      `<p>${escHtml(get(L, 'info.arch_subtitle'))}</p>`,
      `<h3>${escHtml(get(L, 'info.arch_bad_title'))}</h3>`,
      `<ul>${[1, 2, 3].map((i) => `<li>${escHtml(get(L, `info.arch_bad_${i}`))}</li>`).join('')}</ul>`,
      `<h3>${escHtml(get(L, 'info.arch_good_title'))}</h3>`,
      `<ul>${[1, 2, 3].map((i) => `<li>${escHtml(get(L, `info.arch_good_${i}`))}</li>`).join('')}</ul>`,
      `<h2>${escHtml(get(L, 'info.faq_title'))}</h2>`,
      // La FAQ n'existait que dans le JSON-LD, pas dans le texte lisible.
      ...[1, 2, 3, 4, 5, 6]
        .map((i) => [get(L, `info.q${i}`), get(L, `info.a${i}`)])
        .filter(([q, a]) => q && a)
        .map(([q, a]) => `<h3>${escHtml(q)}</h3><p>${escHtml(a)}</p>`),
    ].join('\n');
  ...
  // ── /pricing ──
  {
    const body = [
      ...
      // Le prix n'existait nulle part dans le HTML servi : /pricing rendait
      // 98 mots et aucun montant pour un crawler sans JS.
      `<h2>${escHtml(get(L, 'pricing.h1_start'))} ${escHtml(get(L, 'pricing.h1_accent'))}</h2>`,
      `<p>${escHtml(get(L, 'pricing.h1_desc'))}</p>`,
      `<table><caption>${escHtml(get(L, 'pricing.title'))}</caption>`,
      `<thead><tr><th>${lang === 'en' ? 'Plan' : 'Offre'}</th><th>${lang === 'en' ? 'Price' : 'Prix'}</th><th>${lang === 'en' ? 'Batch' : 'Lot'}</th><th>GPS</th></tr></thead><tbody>`,
      `<tr><td>${escHtml(get(L, 'pricing.standard.name'))}</td><td>${escHtml(get(L, 'pricing.standard.price'))}${escHtml(get(L, 'pricing.standard.period'))}</td><td>${escHtml(get(L, 'pricing.standard.batch'))}</td><td>${escHtml(get(L, 'pricing.standard.gps'))}</td></tr>`,
      `<tr><td>${escHtml(get(L, 'pricing.premium.name'))}</td><td>${escHtml(get(L, 'pricing.premium.price'))}${escHtml(get(L, 'pricing.premium.period'))}</td><td>${escHtml(get(L, 'pricing.premium.batch'))}</td><td>${escHtml(get(L, 'pricing.premium.gps'))}</td></tr>`,
      `</tbody></table>`,
      `<p>${escHtml(get(L, 'pricing.premium.yearly_note'))}</p>`,
      `<p>${escHtml(get(L, 'pricing.trial.cta_signup'))}</p>`,
      `<ul><li>${escHtml(get(L, 'pricing.trust.secure'))}</li><li>${escHtml(get(L, 'pricing.trust.cancel'))}</li><li>${escHtml(get(L, 'pricing.trust.no_fees'))}</li></ul>`,
    ].join('\n');
    emit(lang, '/pricing', {
